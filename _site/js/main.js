...
$(document).ready(function(){
	...
	if (typeof gitJson !== 'undefined'){
		for (var j = 0; j < gitJson.length; j++) {
			...
			if (sortTitle.indexOf("sdk") >= 0 || sortTitle.indexOf("mas") >= 0) {
				//if title matches hardcoded repo title then use these forks/stars
				if (sortTitle.includes("ios") === true){
					//ios stars/forks
					totaliOSStars = totaliOSStars + stars;
					totaliOSForks = totaliOSForks + forks;
					$(".iosRepo .sdkRepoStar").text(totaliOSStars);
					$(".iosRepo .sdkRepoFork").text(totaliOSForks);
				} else if (sortTitle.includes("android") === true){
					totalAndroidStars = totalAndroidStars + stars;
					totalAndroidForks = totalAndroidForks + forks;
					$(".androidRepo .sdkRepoStar").text(totalAndroidStars);
					$(".androidRepo .sdkRepoFork").text(totalAndroidForks);
				...
				} else if (sortTitle.includes("cordova") === true){
					totalCordovaStars = totalCordovaStars + stars;
					totalCordovaForks = totalCordovaForks + forks;
					$(".phpRepo .sdkRepoStar").text(totalCordovaStars);
					$(".phpRepo .sdkRepoFork").text(totalCordovaForks);
				} else if (sortTitle.includes("net") === true){
					//xamarin and dot net
					$(".xamarinRepo .sdkRepoStar").text(stars);
					$(".xamarinRepo .sdkRepoFork").text(forks);
				} else if (sortTitle.includes("win") === true){		
					console.log(sortTitle);			
					totalWindowsStars = totalWindowsStars + stars;
					totalWindowsForks = totalWindowsForks + forks;
					//Unity
					$(".unityRepo .sdkRepoStar").text(totalWindowsStars);
					$(".unityRepo .sdkRepoFork").text(totalWindowsForks);
